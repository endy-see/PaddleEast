...
import data_utils
from data_east import IcdarDataset
import os
import cv2
...
def train(batch_size, total_batch_size=None, padding_total=False, shuffle=True, shuffle_seed=None):
    return icdar('train', batch_size, total_batch_size, padding_total, shuffle=shuffle, shuffle_seed=shuffle_seed)


def icdar(mode, batch_size=None, total_batch_size=None, padding_total=False, shuffle=False, shuffle_seed=None):
    total_batch_size = total_batch_size if total_batch_size else batch_size
    assert total_batch_size % batch_size == 0
    icdar_dataset = IcdarDataset(mode)
    data_nums = icdar_dataset.data_nums()
    ...
    def reader():
        if mode == "train":
            ...
            while True:
                datas = icdar_dataset.getitem(index)
